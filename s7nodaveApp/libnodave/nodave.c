...
/* libnodave casts between structures and buffers, therefore pragma pack might
 * be needed. */
#pragma pack(1)
...
#ifdef HAVE_SELECT
int DAVE_DECL2 stdwrite(daveInterface *di, char *buffer, int length) {
    if (daveDebug & daveDebugByte)
	_daveDump("I send", (unsigned char*)buffer, length);
    return write(di->fd.wfd, buffer,length);
}

int DAVE_DECL2 stdread(daveInterface * di, char * buffer, int length) {
    fd_set FDS;	
    struct timeval t;
    int i;
...
int DAVE_DECL2 stdread(daveInterface * di,
	 char * buffer, 
	 int length) {
    unsigned long i;
...
} daveS7OexchangeBlock;

/* 
    Setup a new interface structure from an initialized serial interface's handle and a name.
*/
daveInterface * DAVE_DECL2 daveNewInterface(daveFileDescriptors nfd, const char * nname, int localMPI, int protocol, int speed){
...
daveInterface * DAVE_DECL2 davePascalNewInterface(daveFileDescriptors* nfd, char * nname, int localMPI, int protocol, int speed){
#ifdef DEBUG_CALLS
...
void DAVE_DECL2 daveSetDebug(int nDebug) {
#ifdef DEBUG_CALLS
    LOG2("daveSetDebug(%d)\n",nDebug);
    FLUSH;
#endif	
...
/**
    C# interoperability:
**/
void DAVE_DECL2 daveSetTimeout(daveInterface * di, int tmo) {
#ifdef DEBUG_CALLS
...
int DAVE_DECL2 daveGetTimeout(daveInterface * di) {
#ifdef DEBUG_CALLS
...
char * DAVE_DECL2 daveGetName(daveInterface * di) {
#ifdef DEBUG_CALLS
...
int DAVE_DECL2 daveGetMPIAdr(daveConnection * dc) {
#ifdef DEBUG_CALLS
...
int DAVE_DECL2 daveGetAnswLen(daveConnection * dc) {
#ifdef DEBUG_CALLS
...
int DAVE_DECL2 daveGetMaxPDULen(daveConnection * dc) {
#ifdef DEBUG_CALLS
...
*/
void DAVE_DECL2 _daveInitPDUheader(davePDU * p, int type) {
    memset(p->header, 0, sizeof(davePDUHeader));
    if (type==2 || type==3)
	p->hlen=12;
    else
	p->hlen=10;
    p->param=p->header+p->hlen;
    ((davePDUHeader*)p->header)->P=0x32;
    ((davePDUHeader*)p->header)->type=type;
    p->dlen=0;
    p->plen=0;
    p->udlen=0;
    p->data=NULL;
...
/*
    ...
    needs valid header
*/
void DAVE_DECL2 _daveAddParam(davePDU * p,unsigned char * param,unsigned short len) {
#ifdef c
    ...
    memcpy(&(((davePDUHeader*)p->header)->plen),&tmplen,sizeof(unsigned short));
#else
    ((davePDUHeader*)p->header)->plen=daveSwapIed_16(len);
#endif    
    p->data=p->param+len;
    p->dlen=0;
}
...
*/
void DAVE_DECL2 _daveAddData(davePDU * p,void * data,int len) {
#ifdef ARM_FIX
    unsigned short tmplen;
#endif    
    ...
    memcpy(&(((davePDUHeader*)p->header)->dlen),&tmplen,sizeof(unsigned short));
#else    
    ((davePDUHeader*)p->header)->dlen=daveSwapIed_16(p->dlen);
#endif    
}
...
*/
void DAVE_DECL2 _daveAddValue(davePDU * p,void * data,int len) {
    unsigned short dCount;
    unsigned char * dtype;
    dtype=p->data+p->dlen-4+1;			/* position of first byte in the 4 byte sequence */
#ifdef ARM_FIX    
    memcpy(&dCount, (p->data+p->dlen-4+2), sizeof(unsigned short));
#else
    dCount=* ((unsigned short *)(p->data+p->dlen-4+2));  /* changed for multiple write */
#endif    
    ...
    dCount=daveSwapIed_16(dCount);
#ifdef ARM_FIX    
    memcpy((p->data+p->dlen-4+2), &dCount, sizeof(unsigned short));
#else
    *((unsigned short *)(p->data+p->dlen-4+2))=dCount;
#endif    
...
*/
void DAVE_DECL2 _daveAddUserData(davePDU * p, unsigned char * da, int len) {
...
void DAVE_DECL2 davePrepareReadRequest(daveConnection * dc, davePDU *p) {
...
davePDU * DAVE_DECL2 daveNewPDU() {
    davePDU * p;
    p=(davePDU*)malloc(sizeof(davePDU));
#ifdef DEBUG_CALLS
    LOG2("daveNewPDU() = %p\n",p);
    FLUSH;
#endif	        
...
void DAVE_DECL2 davePrepareWriteRequest(daveConnection * dc, davePDU *p) {
...
void DAVE_DECL2 daveAddToReadRequest(davePDU *p, int area, int DBnum, int start, int byteCount, int isBit) {
    unsigned char pa[]=	{
	0x12, 0x0a, 0x10,
    ...
    memcpy(&(((davePDUHeader*)p->header)->plen), &tmplen, sizeof(unsigned short));
#else
    ((davePDUHeader*)p->header)->plen=daveSwapIed_16(p->plen);
#endif    
...
void DAVE_DECL2 daveAddVarToReadRequest(davePDU *p, int area, int DBnum, int start, int byteCount) {
#ifdef DEBUG_CALLS
...
void DAVE_DECL2 daveAddBitVarToReadRequest(davePDU *p, int area, int DBnum, int start, int byteCount) {
    daveAddToReadRequest(p, area, DBnum, start, byteCount, 1);
}    

void DAVE_DECL2 daveAddToWriteRequest(davePDU *p, int area, int DBnum, int start, int byteCount,
	void * buffer,
	unsigned char * da,
	int dasize,
	unsigned char * pa,
...
void DAVE_DECL2 daveAddVarToWriteRequest(davePDU *p, int area, int DBnum, int start, int byteCount, void * buffer) {
...
void DAVE_DECL2 daveAddBitVarToWriteRequest(davePDU *p, int area, int DBnum, int start, int byteCount, void * buffer) {
...
*/
int DAVE_DECL2 daveGetPDUerror(davePDU * p) {
#ifdef DEBUG_CALLS
...
*/
int DAVE_DECL2 _daveSetupReceivedPDU(daveConnection * dc, davePDU * p) {
...
int DAVE_DECL2 _daveTestResultData(davePDU * p) {
	int res; /*=daveResCannotEvaluatePDU;*/
...
int DAVE_DECL2 _daveTestReadResult(davePDU * p) {
    if (p->param[0]!=daveFuncRead) return daveResUnexpectedFunc;
    return _daveTestResultData(p);
}

int DAVE_DECL2 _daveTestPGReadResult(davePDU * p) {
    int pres=0;
...
int DAVE_DECL2 _daveTestWriteResult(davePDU * p) {
	int res;/* =daveResCannotEvaluatePDU; */
...
*/
int DAVE_DECL2 _daveMemcmp(unsigned short * a, unsigned char *b, size_t len) {
    unsigned int i;
...
/*
    Hex dump:
*/
void DAVE_DECL2 _daveDump(char * name, void*b,int len) {//void DAVE_DECL2 _daveDump(char * name,unsigned char*b,int len) {
    int j;
    LOG2("%s: ",name);
    if (len>daveMaxRawLen) len=daveMaxRawLen; 	/* this will avoid to dump zillions of chars */
    for (j=0; j<len; j++){
	if((j & 0xf)==0) LOG2("\n%x:",j);
	LOG2("0x%02X,",((unsigned char*)(b))[j]);
    }
    LOG1("\n");
}
...
/*
    Hex dump PDU:
*/
void DAVE_DECL2 _daveDumpPDU(davePDU * p) {
    int i,dl;
    unsigned char * pd;
...
/*
    name Objects:
*/
char * DAVE_DECL2 daveBlockName(unsigned char bn) {
#ifdef DEBUG_CALLS
...
char * DAVE_DECL2 daveAreaName(unsigned char n) {
#ifdef DEBUG_CALLS
...
*/
void DAVE_DECL2 _daveConstructUpload(davePDU *p,char blockType, int blockNr) {
...
void DAVE_DECL2 _daveConstructDoUpload(davePDU * p, int uploadID) {
...
void DAVE_DECL2 _daveConstructEndUpload(davePDU * p, int uploadID) {
...
int DAVE_DECL2 daveStop(daveConnection * dc) {
    int res;
    davePDU p,p2;
#ifdef DEBUG_CALLS
    LOG2("daveStop(dc:%p)\n", dc);
...
int DAVE_DECL2 daveStart(daveConnection*dc) {
    int res;
    davePDU p,p2;
#ifdef DEBUG_CALLS
    LOG2("daveStart(dc:%p)\n", dc);
...
int DAVE_DECL2 daveCopyRAMtoROM(daveConnection * dc) {
    int res;
    davePDU p,p2;
#ifdef DEBUG_CALLS
...
*/
int DAVE_DECL2 daveBuildAndSendPDU(daveConnection * dc, davePDU*p2,unsigned char *pa,int psize, unsigned char *ud,int usize) {
    int res;
    davePDU p;
...
int DAVE_DECL2 daveListBlocksOfType(daveConnection * dc,unsigned char type,daveBlockEntry * buf) {
    int res,i, len;
    davePDU p2;
...
*/
int DAVE_DECL2 daveGetOrderCode(daveConnection * dc,char * buf) {
    int res=0;
    davePDU p2;
...
int DAVE_DECL2 daveReadSZL(daveConnection * dc, int ID, int index, void * buffer, int buflen) {
    int res,len,cpylen;
    int pa7;
//    int pa6;
    davePDU p2;
...
int DAVE_DECL2 daveGetBlockInfo(daveConnection * dc, daveBlockInfo *dbi, unsigned char type, int number)
{
    int res;
...
int DAVE_DECL2 daveListBlocks(daveConnection * dc,daveBlockTypeEntry * buf) {
    int res,i;
    davePDU p2;
...
int DAVE_DECL2 daveReadManyBytes(daveConnection * dc,int area, int DBnum, int start,int len, void * buffer){
    int res, pos, readLen;
    unsigned char * pbuf;
    pos=0;
...
*/
int DAVE_DECL2 daveReadBytes(daveConnection * dc,int area, int DBnum, int start,int len, void * buffer){
    davePDU p1,p2;
    int res;
#ifdef DEBUG_CALLS
...
/*
    ...
    as you do not send further requests.
*/
int DAVE_DECL2 daveReadBits(daveConnection * dc,int area, int DBnum, int start,int len, void * buffer){
    davePDU p1,p2;
    int res;
#ifdef DEBUG_CALLS
...
*/
int DAVE_DECL2 daveExecReadRequest(daveConnection * dc, davePDU *p, daveResultSet* rl){
    davePDU p2;
    unsigned char * q;
    daveResult * cr, *c2;
    int res, i, len, rlen;
...
*/
int DAVE_DECL2 daveExecWriteRequest(daveConnection * dc, davePDU *p, daveResultSet* rl){
    davePDU p2;
    unsigned char * q;
    daveResult * cr, *c2;
    int res, i;
...
int DAVE_DECL2 daveUseResult(daveConnection * dc, daveResultSet * rl, int n){
    daveResult * dr;
#ifdef DEBUG_CALLS
...
void DAVE_DECL2 daveFreeResults(daveResultSet * rl){
    daveResult * r;
    int i;
#ifdef DEBUG_CALLS
...
int DAVE_DECL2 daveGetErrorOfResult(daveResultSet *rs, int number) {
...
daveConnection * DAVE_DECL2 daveNewConnection(daveInterface * di, int MPI, int rack, int slot) {
	...
	switch (di->protocol) {
		...
		dc->PDUstartI= sizeof(daveIBHpacket)+sizeof(daveMPIheader);
		dc->PDUstartO= sizeof(daveIBHpacket)+sizeof(daveMPIheader); // 02/01/2005
		break;
	    case daveProtoPPI_IBH:	
	        dc->maxPDUlength=240;	// limit for NetLink as reported by AFK 
		dc->PDUstartI=14; // sizeof(daveIBHpacket)+7;
		dc->PDUstartO=13;// sizeof(daveIBHpacket)+7; // 02/01/2005
		break;	
...
int DAVE_DECL2 daveWriteManyBytes(daveConnection * dc,int area, int DBnum, int start,int len, void * buffer){
...
int DAVE_DECL2 daveWriteBytes(daveConnection * dc,int area, int DB, int start, int len, void * buffer) {
    davePDU p1,p2;
    int res;
    if (dc->iface->protocol==daveProtoAS511) {	    
...
int DAVE_DECL2 daveWriteBits(daveConnection * dc,int area, int DB, int start, int len, void * buffer) {
    davePDU p1,p2;
    int res;
    p1.header=dc->msgOut+dc->PDUstartO;
    davePrepareWriteRequest(dc,&p1);
...
*/
int DAVE_DECL2 daveSetBit(daveConnection * dc,int area, int DB, int byteAdr, int bitAdr) {
    int a=1;
...
*/
int DAVE_DECL2 daveClrBit(daveConnection * dc,int area, int DB, int byteAdr, int bitAdr) {
    int a=0;
    return daveWriteBits(dc, area, DB, 8*byteAdr+bitAdr, 1, &a);
}


int DAVE_DECL2 initUpload(daveConnection * dc,char blockType, int blockNr, int * uploadID){
    davePDU p1,p2;
    int res;
...
int DAVE_DECL2 doUpload(daveConnection*dc, int * more, unsigned char**buffer, int*len, int uploadID){
    davePDU p1,p2;
    int res, netLen;
    p1.header=dc->msgOut+dc->PDUstartO;
...
int DAVE_DECL2 endUpload(daveConnection*dc, int uploadID){
    davePDU p1,p2;
    int res;
...
*/
char * DAVE_DECL2 daveStrerror(int code) {
    switch (code) {
...
*/
void DAVE_DECL2 daveStringCopy(char * intString, char * extString) {
...
*/
float DAVE_DECL2 daveGetKGAt(daveConnection * dc,int pos) {
    char kgExponent;
    int sign;
    union {
	unsigned char b[4];
...
float DAVE_DECL2 daveGetKG(daveConnection * dc) {
    float f;
...
float DAVE_DECL2 daveGetFloat(daveConnection * dc) {
    union {
	float a;
	unsigned char b[4];
    } f;
...
float DAVE_DECL2 daveGetFloatAt(daveConnection * dc, int pos) {
    union {
	float a;
	unsigned char b[4];
    } f;
...
float DAVE_DECL2 toPLCfloat(float ff) {
#ifdef DAVE_LITTLE_ENDIAN    
    union {
	float a;
	unsigned char b[4];
...
int DAVE_DECL2 daveToPLCfloat(float ff) {
    union {
	float a;
	unsigned char b[4];
	int c;
...
int DAVE_DECL2 daveToKG(float ff) {
    union {
	unsigned char b[4];
	int c;
    } f,f2;
...
short DAVE_DECL2 daveSwapIed_16(short ff) {
#ifdef DAVE_LITTLE_ENDIAN
    union {
	short a;
	unsigned char b[2];
...
int DAVE_DECL2 daveSwapIed_32(int ff) {
#ifdef DAVE_LITTLE_ENDIAN
    union {
	int a;
	unsigned char b[4];
...
*/
float DAVE_DECL2 daveGetSeconds(daveConnection * dc) {
    unsigned char b[2],a;
    float f;
...
*/
float DAVE_DECL2 daveGetSecondsAt(daveConnection * dc, int pos) {
    float f;
    unsigned char b[2],a;
    unsigned char* p=(unsigned char*)dc->_resultPointer;
    p+=pos;
...
*/
int DAVE_DECL2 daveGetCounterValue(daveConnection * dc) {
    unsigned char b[2];
    int f;
    b[1]=*(dc->resultPointer)++;
    b[0]=*(dc->resultPointer)++;
...
*/
int DAVE_DECL2 daveGetCounterValueAt(daveConnection * dc,int pos){
    int f;
    unsigned char b[2];
    unsigned char* p=(unsigned char*)dc->_resultPointer;
    p+=pos;
...
int DAVE_DECL2 _daveReturnOkDummy(daveInterface * di){
    return 0;
}

int DAVE_DECL2 _daveReturnOkDummy2(daveConnection * dc){
    return 0;
}

int DAVE_DECL2 _daveListReachablePartnersDummy (daveInterface * di, char * buf) {
    return 0;
}
...
void DAVE_DECL2 _daveSendSingle(daveInterface * di,	/* serial interface */
...
int DAVE_DECL2 _daveReadSingle(daveInterface * di) {
	char res;
	int i;
...
int DAVE_DECL2 _daveReadMPI(daveInterface * di, unsigned char *b) {
...
rep:	
	{	
	    ...
	    res+=nr_read;
	    if ((res==1) && (*(b+res-1)==DAVE_DLE)) {
		if ((daveDebug & daveDebugSpecialChars)!=0)
		    LOG1("readMPI single DLE.\n");
		return 1;
	    }		
	    if ((res==1) && (*(b+res-1)==DAVE_STX)) {
		if ((daveDebug & daveDebugSpecialChars)!=0)
...
*/		    
	    ...
	    if (*(b+res-1)==DAVE_ETX) if (state==1) {
	        state=3;
	        if ((daveDebug & daveDebugSpecialChars)!=0)
...
int DAVE_DECL2 _daveReadMPI2(daveInterface * di, unsigned char *b) {
	int res=_daveReadMPI(di, b);
	if (res>1) {
		_daveSendSingle(di, DAVE_DLE);
		_daveSendSingle(di, DAVE_STX);
	}
	return res;
}

int DAVE_DECL2 _daveGetAck(daveConnection * dc) {
    int res;    
...
*/
int DAVE_DECL2 _daveReadChars2(daveInterface * di,	/* serial interface */
...
*/
int DAVE_DECL2 _daveSendWithCRC(daveInterface * di, /* serial interface */
...
/* 
    ...
    and adding DLE,ETX and bcc.
*/
int DAVE_DECL2 _daveSendWithPrefix(daveConnection * dc, unsigned char *b, int size)
{
    unsigned char target[daveMaxRawLen];
...
int DAVE_DECL2 _daveSendWithPrefix2(daveConnection * dc, int size)
{
    unsigned char fix[]= {04,0x80,0x80,0x0C,0x03,0x14};
    unsigned char fix2[]= {0x00, 0x0C, 0x03, 0x03};
...
*/
int DAVE_DECL2 _daveSendAck(daveConnection * dc, int nr)
{
    unsigned char m[3];
    if (daveDebug & daveDebugPacket)
...
*/
int DAVE_DECL2 _daveIncMessageNumber(daveConnection * dc) {
...
*/
int DAVE_DECL2 _daveSendDialog2(daveConnection * dc, int size)
{
    int a;
    _daveSendSingle(dc->iface, DAVE_STX);
    if (_daveReadSingle(dc->iface)!=DAVE_DLE) {
...
int DAVE_DECL2 _daveGetResponseMPI(daveConnection *dc) {
    int res;
    ...
    _daveSendSingle(dc->iface,DAVE_DLE);
...
*/
int DAVE_DECL2 _daveSendMessageMPI(daveConnection * dc, davePDU * p) {
    if (daveDebug & daveDebugExchange) {
...
//	return -1;	
    ...
    if (_daveReadSingle(dc->iface)!=DAVE_STX) {
	if (daveDebug & daveDebugPrintErrors) {
...
int DAVE_DECL2 _daveExchangeMPI(daveConnection * dc, davePDU * p) {
...
*/
int DAVE_DECL2 _daveInitStep(daveInterface * di, int nr, unsigned char *fix, int len, char * caller) {
    _daveSendSingle(di, DAVE_STX);
    if (_daveReadSingle(di)!=DAVE_DLE){
    ...
    if (_daveReadSingle(di)!=DAVE_DLE) return nr+1;
    if (daveDebug & daveDebugInitAdapter)
	LOG4("%s %s step %d.\n", di->name, caller,nr+1);
    if (_daveReadSingle(di)!=DAVE_STX) return nr+2;
    if (daveDebug & daveDebugInitAdapter)
	LOG4("%s %s step %d.\n", di->name, caller,nr+2);
    _daveSendSingle(di, DAVE_DLE);
    return 0;
...
*/
int DAVE_DECL2 _daveInitAdapterMPI2(daveInterface * di)  /* serial interface */
{
    unsigned char b3[]={
...
*/
int DAVE_DECL2 _daveInitAdapterMPI1(daveInterface * di) {
    unsigned char b2[]={
	0x01,0x0D,0x02,
    };
...
*/        
	...
	if (_daveReadSingle(di)!=DAVE_DLE) return 8;
	if (daveDebug & daveDebugInitAdapter)
	    LOG2("%s initAdapter() Adapter E=0330 step 8.\n", di->name);
	res= _daveReadMPI(di, b1);
	if (res!=1 || b1[0]!=DAVE_STX) return 9;
	if (daveDebug & daveDebugInitAdapter)
...
/*
unsigned short ccrc(unsigned char *b, int size, unsigned short start) {
    unsigned short sum;
    int i, j;
...
*/
/*
unsigned short startTab[]={0x0000 , // 0
0x0000 , // 1
0x0000 , // 2
0xbdb7 , // 3
0x0000 , // 4
...
*/
int DAVE_DECL2 _daveInitAdapterMPI3(daveInterface * di)
{
...
int DAVE_DECL2 _daveSendWithPrefix32(daveConnection * dc, int size) {
    unsigned char fix[]= {04,0x80,0x80,0x0C,0x03,0x14};
    fix[4]=dc->connectionNumber2;		// 1/10/05 trying Andrew's patch
    fix[5]=dc->connectionNumber;		// 1/10/05 trying Andrew's patch
    memcpy(dc->msgOut, fix, sizeof(fix));
...
int DAVE_DECL2 _daveListReachablePartnersMPI3(daveInterface * di,char * buf) {
    unsigned char b1[daveMaxRawLen];
...
*/
int DAVE_DECL2 _daveConnectPLCMPI3(daveConnection * dc) {
    int res, mpi;
    
    davePDU p1;
    unsigned char b1[daveMaxRawLen];
    
    unsigned char e18[]={0x04,0x82,0x00,
...
/* 
    ...
    and adding DLE,ETX and bcc.
*/
int DAVE_DECL2 _daveSendWithPrefix31(daveConnection * dc, unsigned char *b, int size)
{
    unsigned char target[daveMaxRawLen];
    unsigned char fix[]= {04,0x80,0x80,0x0C,0x03,0x14};
    fix[4]=dc->connectionNumber2; 		// 1/10/05 trying Andrew's patch
...
/*
    Executes part of the dialog necessary to send a message:
*/
int DAVE_DECL2 _daveSendDialog3(daveConnection * dc, int size)
{
    if (size>5){
	dc->needAckNumber=dc->messageNumber;
...
int DAVE_DECL2 _daveSendMessageMPI3(daveConnection * dc, davePDU * p) {
    if (daveDebug & daveDebugExchange) {
...
/* 
    Sends an ackknowledge message for the message number nr:
*/
int DAVE_DECL2 _daveSendAckMPI3(daveConnection * dc, int nr)
{
    unsigned char m[3];
    if (daveDebug & daveDebugPacket)
	LOG3("%s sendAck for message %d \n", dc->iface->name,nr);
...
int DAVE_DECL2 _daveGetResponseMPI3(daveConnection *dc) {
    int res,count;
    if (daveDebug & daveDebugExchange)
...
int DAVE_DECL2 _daveExchangeMPI3(daveConnection * dc, davePDU * p) {
    _daveSendMessageMPI3(dc, p);
    dc->AnswLen=0;
    return _daveGetResponseMPI3(dc);
}

int DAVE_DECL2 _daveDisconnectPLCMPI3(daveConnection * dc)
{
//    unsigned char m[]={
//        0x80
//    };
...
*/
int DAVE_DECL2 _daveDisconnectAdapterMPI3(daveInterface * di) {
//    unsigned char m3[]={
//        0x80
//    };
    unsigned char m2[]={
...
/*
    ...
    from adapter are not as expected.
*/
int DAVE_DECL2 _daveDisconnectAdapterMPI(daveInterface * di) {
    int res;
    unsigned char m2[]={
        1,4,2
    };
    ...
    _daveSendSingle(di, DAVE_STX);
    res=_daveReadMPI(di,b1);
/*    if ((res!=1)||(b1[0]!=DAVE_DLE)) return -1; */
    _daveSendWithCRC(di, m2, sizeof(m2));		
    if (daveDebug & daveDebugInitAdapter) 
	LOG2("%s daveDisconnectAdapter() step 1.\n", di->name);	
    res=_daveReadMPI(di, b1);
/*    if ((res!=1)||(b1[0]!=DAVE_DLE)) return -2; */
    res=_daveReadMPI(di, b1);
/*    if ((res!=1)||(b1[0]!=DAVE_STX)) return -3; */
    if (daveDebug & daveDebugInitAdapter) 
...
*/
int DAVE_DECL2 _daveListReachablePartnersMPI(daveInterface * di,char * buf) {
    unsigned char b1[daveMaxRawLen];
    unsigned char m1[]={1,7,2};
    int res;
...
int DAVE_DECL2 _daveDisconnectPLCMPI(daveConnection * dc)
{
    int res;
    unsigned char m[]={
        0x80
    ...
    res=_daveReadMPI(dc->iface,b1);
    if ((res!=1)||(b1[0]!=DAVE_STX)) return 6;
    if (daveDebug & daveDebugConnect) 
...
*/
int DAVE_DECL2 _daveNegPDUlengthRequest(daveConnection * dc, davePDU *p) {
...
/* 
    ...
    (or reused for the same PLC ?)
*/
int DAVE_DECL2 _daveConnectPLCMPI2(daveConnection * dc) {
    int res;
    davePDU p1;
    unsigned char b1[daveMaxRawLen];
    ...
    res=_daveReadMPI(dc->iface,b1);
    if ((res!=1)||(b1[0]!=DAVE_DLE)) return 5;
    res=_daveReadMPI(dc->iface,b1);
    if ((res!=1)||(b1[0]!=DAVE_STX)) return 5;
    ...
    res=_daveReadMPI(dc->iface,b1);
    _daveSendSingle(dc->iface, DAVE_DLE);
    if (dc->iface->protocol==daveProtoMPI4) _daveSendSingle(dc->iface, DAVE_STX);
...
/* 
    ...
    (or reused for the same PLC ?)
*/
int DAVE_DECL2 _daveConnectPLCMPI1(daveConnection * dc) {
    int res;
    davePDU p1;
    unsigned char b4[]={
    ...
    if (_daveReadSingle(dc->iface)!=DAVE_DLE) return 4;
    if (daveDebug & daveDebugConnect) 
	LOG2("%s daveConnectPLC() step 5.\n", dc->iface->name);	
    _daveSendWithPrefix(dc, b5, sizeof(b5));		
    if (_daveReadSingle(dc->iface)!=DAVE_DLE) return 5;
    if (_daveReadSingle(dc->iface)!=DAVE_STX) return 5;
...
*/
#ifdef HAVE_SELECT
int DAVE_DECL2 _daveReadOne(daveInterface * di, unsigned char *b) {
	fd_set FDS;
	struct timeval t;
	FD_ZERO(&FDS);
	FD_SET(di->fd.rfd, &FDS);
...
*/
int DAVE_DECL2 _daveReadISOPacket(daveInterface * di,unsigned char *b) {
	int res,length;
	fd_set FDS;
	struct timeval t;
	FD_ZERO(&FDS);
...
/*
    ...
    fd_set FDS,EFDS;
*/
int DAVE_DECL2 _daveReadIBHPacket(daveInterface * di,unsigned char *b) {
    struct timeval t;
    fd_set FDS,EFDS;
    int res,length;
    t.tv_sec = di->timeout / 1000000;
...
int DAVE_DECL2 _daveSendISOPacket(daveConnection * dc, int size) {
    unsigned long i;
    size+=4;
...
int DAVE_DECL2 _daveGetResponseISO_TCP(daveConnection * dc) {
    int res;
...
*/
int DAVE_DECL2 _daveExchangeTCP(daveConnection * dc, davePDU * p) {
    int res;
    if (daveDebug & daveDebugExchange) {
...
int DAVE_DECL2 _daveConnectPLCTCP(daveConnection * dc) {
...
void DAVE_DECL2 _daveSendLength(daveInterface * di, int len) {
...
void DAVE_DECL2 _daveSendIt(daveInterface * di, unsigned char * b, int size) {
    int i;
...
void DAVE_DECL2 _daveSendRequestData(daveConnection * dc,int alt) {
    unsigned char b[]={DAVE_DLE,0,0,0x5C,0,0};
    b[1]=dc->MPIAdr;
...
int DAVE_DECL2 _daveGetResponsePPI(daveConnection *dc) {
...
int DAVE_DECL2 _daveExchangePPI(daveConnection * dc,davePDU * p1) {
    int i,res=0,len;
...
int DAVE_DECL2 _daveConnectPLCPPI(daveConnection * dc) {
    davePDU p;
...
*/
int DAVE_DECL2 daveInitAdapter(daveInterface * di) {
    return di->initAdapter(di);
}

int DAVE_DECL2 daveConnectPLC(daveConnection * dc) {
    return dc->iface->connectPLC(dc);
}

int DAVE_DECL2 daveDisconnectPLC(daveConnection * dc) {
    return dc->iface->disconnectPLC(dc);
}

int DAVE_DECL2 daveDisconnectAdapter(daveInterface * di) {
    return di->disconnectAdapter(di);
}

int DAVE_DECL2 _daveExchange(daveConnection * dc, davePDU *p) {
    int res;
...
int DAVE_DECL2 daveSendMessage(daveConnection * dc, davePDU *p) {
    return dc->iface->sendMessage(dc, p);
}

int DAVE_DECL2 daveListReachablePartners(daveInterface * di, char * buf) {
    return di->listReachablePartners(di, buf);
}

int DAVE_DECL2 daveGetResponse(daveConnection * dc) {
...
*/
int DAVE_DECL2 daveGetS8from(unsigned char *b) {
    char* p=(char*)b;
    return *p;
}

int DAVE_DECL2 daveGetU8from(unsigned char *b) {
    return *b;
}

int DAVE_DECL2 daveGetS16from(unsigned char *b) {
    union {
	short a;
	unsigned char b[2];
    } u;
...
int DAVE_DECL2 daveGetU16from(unsigned char *b) {
    union {
	unsigned short a;
	unsigned char b[2];
    } u;
...
int DAVE_DECL2 daveGetS32from(unsigned char *b) {
    union {
	int a;
	unsigned char b[4];
    } u;
...
unsigned int DAVE_DECL2 daveGetU32from(unsigned char *b) {
    union {
	unsigned int a;
	unsigned char b[4];
    } u;
...
float DAVE_DECL2 daveGetFloatfrom(unsigned char *b) {
    union {
	float a;
	unsigned char b[4];
    } u;
...
*/
int DAVE_DECL2 daveGetS8(daveConnection * dc) {
    char * p;
#ifdef DEBUG_CALLS
    LOG2("daveGetS8(dc:%p)\n",dc);
    FLUSH;
...
int DAVE_DECL2 daveGetU8(daveConnection * dc) {
    unsigned char * p;
#ifdef DEBUG_CALLS
    LOG2("daveGetU8(dc:%p)\n",dc);
    FLUSH;
...
int DAVE_DECL2 daveGetS16(daveConnection * dc) {
    union {
	short a;
	unsigned char b[2];
    } u;
...
int DAVE_DECL2 daveGetU16(daveConnection * dc) {
    union {
	unsigned short a;
	unsigned char b[2];
    } u;
...
int DAVE_DECL2 daveGetS32(daveConnection * dc) {
    union {
	int a;
	unsigned char b[4];
    } u;
...
unsigned int DAVE_DECL2 daveGetU32(daveConnection * dc) {
    union {
	unsigned int a;
	unsigned char b[4];
    } u;
...
*/
int DAVE_DECL2 daveGetS8At(daveConnection * dc, int pos) {
    char * p=(char *)(dc->_resultPointer);
    p+=pos;
    return *p;
}

int DAVE_DECL2 daveGetU8At(daveConnection * dc, int pos)  {
    unsigned char * p=(unsigned char *)(dc->_resultPointer);
    p+=pos;
    return *p;
}

int DAVE_DECL2 daveGetS16At(daveConnection * dc, int pos) {
    union {
	short a;
	unsigned char b[2];
    } u;
...
int DAVE_DECL2 daveGetU16At(daveConnection * dc, int pos) {
    union {
	unsigned short a;
	unsigned char b[2];
    } u;
...
int DAVE_DECL2 daveGetS32At(daveConnection * dc, int pos) {
    union {
	int a;
	unsigned char b[4];
    } u;
...
unsigned int DAVE_DECL2 daveGetU32At(daveConnection * dc, int pos) {
    union {
	unsigned int a;
	unsigned char b[4];
    } u;
...
*/
unsigned char * DAVE_DECL2 davePut8(unsigned char *b,int v) {
    *b = v & 0xff;
    b++;
    return b;
}

unsigned char * DAVE_DECL2 davePut16(unsigned char *b,int v) {
    union {
	short a;
	unsigned char b[2];
    } u;
...
unsigned char * DAVE_DECL2 davePut32(unsigned char *b, int v) {
    union {
	int a;
	unsigned char b[4];
    } u;
...
unsigned char * DAVE_DECL2 davePutFloat(unsigned char *b,float v) {
    union {
	float a;
	unsigned char b[4];
    } u;
...
void DAVE_DECL2 davePut8At(unsigned char *b, int pos, int v) {
    union {
	short a;
	unsigned char b[2];
    } u;
...
void DAVE_DECL2 davePut16At(unsigned char *b, int pos, int v) {
    union {
	short a;
	unsigned char b[2];
    } u;
...
void DAVE_DECL2 davePut32At(unsigned char *b, int pos, int v) {
    union {
	int a;
	unsigned char b[4];
    } u;
...
void DAVE_DECL2 davePutFloatAt(unsigned char *b, int pos,float v) {
    union {
	float a;
	unsigned char b[4];
    } u;
...
*/
daveUserReadFunc daveReadCallBack=NULL;
daveUserWriteFunc daveWriteCallBack=NULL;

void DAVE_DECL2 _daveConstructReadResponse(davePDU * p) {
...
void DAVE_DECL2 _daveConstructBadReadResponse(davePDU * p) {
    unsigned char pa[]={4,1};
...
void DAVE_DECL2 _daveConstructWriteResponse(davePDU * p) {
...
void DAVE_DECL2 _daveHandleRead(davePDU * p1,davePDU * p2) {
    int result;
    ...
    if (daveReadCallBack)
	userBytes=daveReadCallBack(area, DBnumber,start, bytes, &result);
    _daveConstructReadResponse(p2);	
    _daveAddValue(p2, userBytes, bytes);
    _daveDumpPDU(p2);
};

void DAVE_DECL2 _daveHandleWrite(davePDU * p1,davePDU * p2) {
	...
	bytes, daveAreaName(area),DBnumber,start);
    if (daveWriteCallBack)
	daveWriteCallBack(area, DBnumber,start, bytes, &result, p1->data+4);
    LOG1("after callback\n");
    FLUSH;
...
int DAVE_DECL2 _daveWriteIBH(daveInterface * di, unsigned char * buffer, int len) { //cs: let it be uc
    int res;
    if (daveDebug & daveDebugByte) {
...
int DAVE_DECL2 _davePackPDU(daveConnection * dc,davePDU *p) {
    daveIBHpacket * ibhp;
    daveMPIheader * hm= (daveMPIheader*) (dc->msgOut+sizeof(daveIBHpacket)); // MPI headerPDU begins packet header
    hm->MPI=dc->MPIAdr;
...
void DAVE_DECL2 _daveSendMPIAck_IBH(daveConnection*dc) {
...
*/
void DAVE_DECL2 _daveSendIBHNetAck(daveConnection * dc) {
    daveIBHpacket * p;
    unsigned char ack[13];
    memcpy(ack, dc->msgIn, sizeof(ack));
    p= (daveIBHpacket*) ack;
    p->len=sizeof(ack)-sizeof(daveIBHpacket);
    ack[11]=1;
    ack[12]=9;
...
*/
int DAVE_DECL2 __daveAnalyze(daveConnection * dc) {
    int haveResp;
    daveIBHpacket * p, *p2;
    daveMPIheader * pm;
    daveMPIheader2 * m2;
...
//    if (dc->AnswLen==0) return _davePtEmpty;
    ...
    if (p->rFlags==0x82) {
	pm= (daveMPIheader*) (dc->msgIn+sizeof(daveIBHpacket));
...
#ifdef passiveMode
// construct response:	    
	    pr.header=resp+sizeof(daveIBHpacket)+sizeof(daveMPIheader2);
#endif	    
	    p2= (daveIBHpacket*) resp;
	    p2->ch1=p->ch2;
	    p2->ch2=p->ch1;
	    p2->packetNumber=0;
	    p2->sFlags=0;
	    ...
	    m2= (daveMPIheader2*) (resp+sizeof(daveIBHpacket));
    ...
    if (p->rFlags==0x2c2) {
	daveMPIheader2 * pm= (daveMPIheader2*) (dc->msgIn+sizeof(daveIBHpacket));
	if (daveDebug & daveDebugMPI) {
	    LOG2("srcconn: %d\n",pm->src_conn);
	    LOG2("dstconn: %d\n",pm->dst_conn);
	    LOG2("MPI:     %d\n",pm->MPI);
	...
	if (pm->func==0xf1) {
		...
		LOG1("analyze 1\n");
	    dc->needAckNumber=pm->packetNumber;
	    dc->PDUstartI= sizeof(daveIBHpacket)+sizeof(daveMPIheader2);
...
/*	
	    ...
	    dc->needAckNumber=pm->packetNumber;
//	    p1.header=((unsigned char*)pm)+sizeof(daveMPIheader2);
	    dc->PDUstartI= sizeof(daveIBHpacket)+sizeof(daveMPIheader2);
	    _daveSetupReceivedPDU(dc, &p1);
...
int DAVE_DECL2 _daveExchangeIBH(daveConnection * dc, davePDU * p) {
...
int DAVE_DECL2 _daveSendMessageMPI_IBH(daveConnection * dc, davePDU * p) {
    int res;
    _davePackPDU(dc, p);
...
int DAVE_DECL2 _daveGetResponseMPI_IBH(daveConnection * dc) {
    int res,count,pt;
    count=0;
    pt=0;
    do {
...
*/
int DAVE_DECL2 _daveInitStepIBH(daveInterface * iface, unsigned char * chal, int cl, unsigned short* resp,int rl, unsigned char*b) {
    int res, res2, a=0;
    if (daveDebug & daveDebugConnect) 
...
*/
int DAVE_DECL2 _daveConnectPLC_IBH(daveConnection*dc) {
    int a, retries;
    davePDU p1;
    unsigned char b[daveMaxRawLen];
...
*/
int DAVE_DECL2 _daveDisconnectPLC_IBH(daveConnection*dc) {
    unsigned char b[daveMaxRawLen];
...
*/
int DAVE_DECL2 daveForceDisconnectIBH(daveInterface * di, int src, int dest, int mpi) {
    unsigned char b[daveMaxRawLen];
    chal31[8]=src;
    chal31[9]=dest;
    chal31[10]=mpi;
...
/*
    ...
    Returns 0 for success and a negative number on errors.
*/
int DAVE_DECL2 daveResetIBH(daveInterface * di) {
...
void DAVE_DECL2 _daveSendMPIAck2(daveConnection *dc) {
    daveIBHpacket * p;
    unsigned char c;
...
int DAVE_DECL2 _davePackPDU_PPI(daveConnection * dc,davePDU *p) {
    daveIBHpacket * ibhp;
    unsigned char IBHPPIHeader[]={0xff,0xff,2,0,8};
    IBHPPIHeader[2]=dc->MPIAdr;
    memcpy(dc->msgOut+sizeof(daveIBHpacket), &IBHPPIHeader, sizeof(IBHPPIHeader));
...
//    dc->msgOut[3]=2;
    *(dc->msgOut+sizeof(daveIBHpacket)+4)=p->hlen+p->plen+p->dlen+0;
    ibhp->packetNumber=dc->packetNumber;
    ibhp->rFlags=0x80;
    ibhp->sFlags=0;
...
/*
    send a network level ackknowledge
*/
void DAVE_DECL2 _daveSendIBHNetAckPPI(daveConnection * dc) {
...
int DAVE_DECL2 _daveListReachablePartnersMPI_IBH(daveInterface * di, char * buf) {
    int a, i;
...
/*
    packet analysis. mixes all levels.
*/
int DAVE_DECL2 __daveAnalyzePPI(daveConnection * dc, unsigned char sa) {
    daveIBHpacket* p;
...
int DAVE_DECL2 _daveExchangePPI_IBH(daveConnection * dc, davePDU * p) {
    int res, count, pt;
...
int DAVE_DECL2 _daveGetResponsePPI_IBH(daveConnection * dc) {
    int res, count, pt;
    count=0;
    do {
	_daveSendIBHNetAckPPI(dc);
...
*/
int DAVE_DECL2 BuildAndSendPDU(daveConnection * dc, davePDU*p2,unsigned char *pa,int psize, unsigned char *ud,int usize,
...
//    LOG2("*** here we are: %d\n",p.dlen);
    ...
    ((davePDUHeader*)p3->header)->dlen=daveSwapIed_16(p3->dlen);
...
int DAVE_DECL2 daveForce200(daveConnection * dc,int area, int start, int val) {
    int res;
    davePDU p2;
//    unsigned char pa[]={0,1,18,4,17,67,2,0};
//    unsigned char da[]={'0','0'};
...
daveResultSet * DAVE_DECL2 daveNewResultSet() {
...
void DAVE_DECL2 daveFree(void * dc) {
...
int DAVE_DECL2 daveGetProgramBlock(daveConnection * dc, int blockType, int number, char* buffer, int * length) {
...
int DAVE_DECL2 daveReadPLCTime(daveConnection * dc) {
    int res, len;
    davePDU p2;
...
int DAVE_DECL2 daveSetPLCTime(daveConnection * dc,unsigned char * ts) {
    int res, len;
    davePDU p2;
...
unsigned char DAVE_DECL2 daveToBCD(unsigned char i) {
    return 16*(i /10)+(i%10);
}

unsigned char DAVE_DECL2 daveFromBCD(unsigned char i) {
    return 10*(i /16)+(i%16);
}

int DAVE_DECL2 daveSetPLCTimeToSystime(daveConnection * dc) {
    int res, len;
    davePDU p2;
    unsigned char pa[]={0,1,18,4,17,'G',2,0};
    unsigned char ts[]={
...
unsigned char __davet2[]={DAVE_STX};
unsigned char __davet10[]={DAVE_DLE};
char __davet1006[]={DAVE_DLE,DAVE_ACK};  // cs: this is only sent by system functions, so let it be char
unsigned short __daveT1006[]={DAVE_DLE,DAVE_ACK};
unsigned char __davet121003[]={0x12,DAVE_DLE,DAVE_ETX};
unsigned short __daveT121003[]={0x12,DAVE_DLE,DAVE_ETX};
unsigned char __davet161003[]={0x16,DAVE_DLE,DAVE_ETX};
unsigned short __daveT161003[]={0x16,DAVE_DLE,DAVE_ETX};
/*
...
*/
int DAVE_DECL2 daveReadS5Bytes(daveConnection * dc, unsigned char area, unsigned char BlockN, int offset, int count)
{
...
*/
void DAVE_DECL2 _daveSendDLEACK(daveInterface * di)	// serial interface
{
...
*/
int DAVE_DECL2 _daveSendWithDLEDup(daveInterface * di, // serial interface
		       unsigned char *b, 		    // a buffer containing the message
		       int size		    // the size of the string
		   )
{		
...
*/
int DAVE_DECL2 _daveReqTrans(daveConnection * dc, unsigned char trN)
{
    unsigned char b1[3];
    int res;
    if (daveDebug & daveDebugExchange)
...
*/
int DAVE_DECL2 _daveEndTrans(daveConnection * dc)
{
    int res;
    unsigned char b1[3];
    if (daveDebug & daveDebugExchange)
...
*/
int DAVE_DECL2 _daveDLEDeDup(daveConnection * dc, unsigned char* rawBuf, int rawLen) {
    int j=0,k;
    for (k=0;k<rawLen-2;k++){
	dc->msgIn[j]=rawBuf[k]; j++;
	if (DAVE_DLE==rawBuf[k]){
	    if (DAVE_DLE!=rawBuf[k+1]) return -1;//Bad doubling found
	    k++;
	}
    }
...
int DAVE_DECL2 _daveExchangeAS511(daveConnection * dc, unsigned char * b, int len, int maxlen, int trN) {
    int res, i;
    unsigned char b1[3];
...
//	usleep(500000);
	...
	} while((i>0)&& ( (dc->msgIn[res-2]!=DAVE_DLE) || (dc->msgIn[res-1]!=DAVE_ETX)));
	...
	if ((dc->msgIn[res-2]!=DAVE_DLE)||(dc->msgIn[res-1]!=DAVE_ETX)) {
...
*/
int DAVE_DECL2 _daveReadS5BlockAddress(daveConnection * dc, unsigned char area, unsigned char BlockN, daveS5AreaInfo * ai)
{
...
int DAVE_DECL2 _daveIsS5BlockArea(unsigned char area)
{
    if (
...
int DAVE_DECL2 _daveIsS5DBlockArea(unsigned char area)
{
    if (area!=daveDB) {
...
*/
int DAVE_DECL2 _daveFakeExchangeAS511(daveConnection * dc, davePDU *p){
...
int DAVE_DECL2 _daveConnectPLCAS511(daveConnection * dc){
    int res;
...
int DAVE_DECL2 _daveDisconnectPLCAS511(daveConnection * dc){
    free(dc->cache);
    dc->cache=0;
    return 0;
}
...
*/
int DAVE_DECL2 daveWriteS5Bytes(daveConnection * dc, unsigned char area, unsigned char BlockN, int offset, int count, void * buf)
{
    int res,datastart;
    daveS5AreaInfo ai;
    unsigned char b1[daveMaxRawLen];
...
int DAVE_DECL2 daveStopS5(daveConnection * dc) {
    unsigned char b1[]={0x88,0x04};	// I don't know what this mean
    return daveWriteBytes(dc,daveSysDataS5,0,0x0c,2,b1);
}

int DAVE_DECL2 daveStartS5(daveConnection * dc) {
    unsigned char b1[]={0x68,0x00};	// I don't know what this mean
    return daveWriteBytes(dc,daveSysDataS5,0,0x0c,2,b1);
}

int DAVE_DECL2 daveGetS5ProgramBlock(daveConnection * dc, int blockType, int number, char* buffer, int * length) {
//    int totlen,res;
//    *length=totlen;
...
*/
int DAVE_DECL2 _daveSCP_send(int fd, unsigned char * reqBlock) {
    daveS7OexchangeBlock* fdr;
    fdr=(daveS7OexchangeBlock*)reqBlock;
    fdr->headerlength=80;
    fdr->allways2 = 2;
    fdr->payloadStart= 80;
...
int daveSCP_receive(int h, unsigned char * buffer) {
    int res, datalen;
    daveS7OexchangeBlock * fdr;
    fdr=(daveS7OexchangeBlock*) buffer;
    res=SCP_receive(h, 0xFFFF, &datalen, sizeof(daveS7OexchangeBlock), buffer);
    if (daveDebug & daveDebugByte) {
...
int DAVE_DECL2 _daveConnectPLCS7online (daveConnection * dc) {
    ...
    daveS7OexchangeBlock * fdr;
    fdr=(daveS7OexchangeBlock*)(dc->msgOut);
    p2[9]=dc->MPIAdr;	///
    dc->PDUstartI=80;
...
int DAVE_DECL2 _daveSendMessageS7online(daveConnection *dc, davePDU *p) {
    int a;
    int datalen;
    int len=p->hlen+p->plen+p->dlen;
    unsigned char buffer[sizeof(daveS7OexchangeBlock)];
    daveS7OexchangeBlock* fdr;
    fdr=(daveS7OexchangeBlock*)dc->msgOut;
...
//    memcpy(&(fdr->payload),buffer,len);
    a= _daveSCP_send((int)(dc->iface->fd.wfd), dc->msgOut);
    SCP_receive((int)(dc->iface->fd.rfd), 0xFFFF, &datalen, sizeof(daveS7OexchangeBlock), buffer);
//    daveSCP_receive(dc->iface->fd.rfd, dc->msgIn);
    return 0;
}    

int DAVE_DECL2 _daveGetResponseS7online(daveConnection *dc) {
    int a;
...
int DAVE_DECL2 _daveExchangeS7online(daveConnection * dc, davePDU * p) {
    int res;
...
int DAVE_DECL2 _daveListReachablePartnersS7online (daveInterface * di, char * buf) {
    int a;
    daveS7OexchangeBlock reqBlock;
    unsigned char b1[sizeof(daveS7OexchangeBlock)];
    daveS7OexchangeBlock* fdr;
    fdr=&reqBlock;
...
/*
int DAVE_DECL2 _daveDisconnectAdapterS7online(daveInterface * di) {
    int res;
...
#ifndef NET
int DAVE_DECL2 _daveReadMPINLpro(daveInterface * di, unsigned char *b) {
	int res=0,state=0,nr_read;
	unsigned char bcc=0;
...
int DAVE_DECL2 _daveInitAdapterNLpro(daveInterface * di)  /* serial interface */
{
    unsigned char b3[]={
...
void DAVE_DECL2 _daveSendSingleNLpro(daveInterface * di,	/* serial interface */
		     unsigned char c  			/* chracter to be send */
		     ) 
{
    unsigned long i;
...
/* 
    ...
    and adding DLE,ETX and bcc.
*/
int DAVE_DECL2 _daveSendWithCRCNLpro(daveInterface * di, /* serial interface */
		    unsigned char *b, 		 /* a buffer containing the message */
		    int size		 /* the size of the string */
		)
{		
...
/* 
    Send a string of init data to the MPI adapter.
*/
int DAVE_DECL2 _daveInitStepNLpro(daveInterface * di, int nr, unsigned char *fix, int len, char * caller, unsigned char * buffer ) {
...
/* 
    ...
    (or reused for the same PLC ?)
*/
int DAVE_DECL2 _daveConnectPLCNLpro(daveConnection * dc) {
    int res;
    davePDU p1;
    unsigned char b4[]={
	0x04,0x80,0x80,0x0D,0x00,0x14,0xE0,0x04,
...
/*
    Executes part of the dialog necessary to send a message:
*/
int DAVE_DECL2 _daveSendDialogNLpro(daveConnection * dc, int size)
{
    if (size>5){
	dc->needAckNumber=dc->messageNumber;
	dc->msgOut[dc->iface->ackPos+1]=_daveIncMessageNumber(dc);
...
/*
    Sends a message and gets ackknowledge:
*/
int DAVE_DECL2 _daveSendMessageNLpro(daveConnection * dc, davePDU * p) {
    if (daveDebug & daveDebugExchange) {
...
int DAVE_DECL2 _daveExchangeNLpro(daveConnection * dc, davePDU * p) {
    _daveSendMessageNLpro(dc, p);
    dc->AnswLen=0;
    return _daveGetResponseNLpro(dc);
}

int DAVE_DECL2 _daveGetResponseNLpro(daveConnection *dc) {
    int res;
    if (daveDebug & daveDebugExchange) {
...
int DAVE_DECL2 _daveSendWithPrefixNLpro(daveConnection * dc, unsigned char *b, int size)
{
    unsigned char target[daveMaxRawLen];
//    unsigned char fix[]= {04,0x80,0x80,0x0C,0x03,0x14};
...
int DAVE_DECL2 _daveSendWithPrefix2NLpro(daveConnection * dc, int size)
{
//    unsigned char fix[]= {04,0x80,0x80,0x0C,0x03,0x14};
...
int DAVE_DECL2 _daveDisconnectPLCNLpro(daveConnection * dc)
{
    int res;
    unsigned char m[]={
        0x80
    ...
    _daveSendSingleNLpro(dc->iface, DAVE_STX);
...
/*
    ...
    from adapter are not as expected.
*/
int DAVE_DECL2 _daveDisconnectAdapterNLpro(daveInterface * di) {
    int res;
    unsigned char m2[]={
        1,4,2
    };
    ...
    if (daveDebug & daveDebugInitAdapter) 
	LOG2("%s enter DisconnectAdapter()\n", di->name);	
//    _daveSendSingleNLpro(di, DAVE_STX);
//    res=_daveReadMPINLpro(di,b1);
/*    if ((res!=1)||(b1[0]!=DAVE_DLE)) return -1; */
...
/*
*/
int DAVE_DECL2 _daveListReachablePartnersNLpro(daveInterface * di,char * buf) {
    unsigned char b1[daveMaxRawLen];
    unsigned char m1[]={1,7,2};
    int res;
