...
typedef S7nodavePortDriverReadOptimizer Optimizer;
typedef Optimizer::ReadItem ReadItem;
typedef Optimizer::ReadItemList ReadItemList;
...
void Optimizer::optimizeWithinArea(ReadItemList& items, int maxPayloadSize)
{
    ...
    while (!items.empty()) {
        ReadItemList::iterator firstItemIterator = items.begin();
...
{
    typedef tuple<s7nodavePlcArea, int> AreaTuple;
    typedef map<AreaTuple, ReadItemList> AreaToItemsMap;
    ...
    while (item != NULL) {
        ...
        AreaTuple key = tuple<s7nodavePlcArea, int>(item->area, item->areaNumber);
        AreaToItemsMap::iterator entry = areaToItems.find(key);
    ...
    for (AreaToItemsMap::iterator i = areaToItems.begin(); i != areaToItems.end(); i++) {
        s7nodavePlcArea area = i->first.get<0>();
...
{
    ReadItemList::iterator it = itemList.begin();
